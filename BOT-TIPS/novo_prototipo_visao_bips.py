...
async def gerar_horarios(inicio, fim, chat_name):
    ...
    try:
        ...
        while inicio <= fim:
            ...
            inicio += timedelta(minutes=random.randint(2,5))
        # print(horarios)
...
async def start_bot(chat_id, chat_name, context):
    ...
    if nova_hora_inicio and nova_hora_fim:
        ...
        # print(f'Horáridos do bot: {horarios}')
        ...
        try:
           ...
           SELECT * FROM TLG_BIPNOTIFIC WHERE COD_GRUPO = %s  AND CODFUN IS NULL ORDER BY HORA_ENVIO DESC LIMIT 1
            ...
            if resposta_funcionario:
                ...
                try:
                    # cursor.execute('SELECT CODE FROM PUBLIC.USER WHERE CODE =%s', (codigo,))
                    cursor.execute('SELECT CODE FROM USERS WHERE CODE =%s', (str(codigo),))
                    funcionario_codigo = cursor.fetchone()
                    # print(f"Resultado da consulta em user: {funcionario_codigo}")
                    if funcionario_codigo:
                        ...
                        await context.bot.send_message(
                                            ...
                                            text=f"Código de resposta inválido no grupo {chat_name}. Favor verificar.")
                except Exception as e:
...
def main() -> None:
    ...
    try:
        ...
        # conexao_api.add_handler(CommandHandler('bip', start_bot))
...
# async def start_bot(chat_id, chat_name, context):
#     global ultima_hora_inicio, ultima_hora_fim, ultima_checagem
#     database = await conexao_banco()
#     cursor = database.cursor()
...
#         ultima_hora_inicio, ultima_hora_fim = nova_hora_inicio, nova_hora_fim
#     else:
#         horarios = []  # Caso não tenha horários no banco, deve continuar sem gerar horários.

#     # Armazenando os horários específicos para cada chat
#     active_chats[chat_id] = {"horarios": horarios, "ultima_hora_inicio": nova_hora_inicio, "ultima_hora_fim": nova_hora_fim, "ultima_checagem": datetime.now()}
...
#     while True:
#         agora = datetime.now()
#         print(f'Última checagem para o grupo {chat_name}: {active_chats[chat_id]["ultima_checagem"]}')

#         # Checando por atualizações no banco
#         if (agora - active_chats[chat_id]["ultima_checagem"]).total_seconds() > 120:
#             print("Checando por atualizações no banco...")
#             nova_hora_inicio, nova_hora_fim = await consulta_banco(chat_id)
...
#         # Verificar se a hora atual está na lista de horários do grupo específico
#         if agora.strftime("%H:%M:%S") in active_chats[chat_id]["horarios"]:
#             print(f"Enviando mensagem para o grupo {chat_name}: {tlg_bip_dados[6]}")
#             await context.bot.send_message(chat_id=chat_id, text=tlg_bip_dados[6])
#             cursor.execute('''INSERT INTO TLG_BIPNOTIFIC (ID_BIP, COD_GRUPO, COD_SUPERVISAO, HORA_ENVIO) VALUES (%s, %s, %s, %s)''',
...
#         else:
#             print(f"Fora do horário para o grupo {chat_name}: {agora.strftime('%H:%M:%S')}")
        
#         await asyncio.sleep(1)

#         # Se estamos no último horário, gerar novos horários
#         if agora.strftime("%H:%M:%S") == active_chats[chat_id]["horarios"][-1]:
#             print(f"Último horário para o grupo {chat_name}: {active_chats[chat_id]['horarios'][-1]}")
#             novos_horarios = await gerar_horarios(active_chats[chat_id]["ultima_hora_inicio"], active_chats[chat_id]["ultima_hora_fim"], chat_name)
#             active_chats[chat_id]["horarios"] = novos_horarios
